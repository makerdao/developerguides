...
Trading on decentralized exchanges involves signing multiple transactions with your wallet.This leads to having higher chances to have failed transactions. Using a proxy simplifies this process by combining many transactions into one.

  

This guide will show you how to make trades on the OasisDEX Protocol through the [Oasis Direct Proxy](https://github.com/makerdao/oasis-direct-proxy). The Oasis Direct Proxy allows you to bundle together many function calls into one function call. So, when you have to swap some ERC-20 tokens, you will be able to do it all in one transaction.

  

This can be done through the DS-Proxy contract, which is a generic mechanism that allows for bundling transactions and having allowances setup once for different contracts. Or creating your own proxy contract that could interact with the Oasis Direct Proxy contract. Another example of proxy contract is [https://instadapp.io/](https://instadapp.io/), where they have created their own proxy contract that interacts with many services.

  

Using a proxy contract is beneficial for you as a user and developers that want to build DApps. The most obvious use case is the front-end applications that interact with the protocol. A good example is the [oasis.app](https://oasis.app/) and the [Dai.js SDK](https://github.com/makerdao/dai.js).
...
Proxy Registry - Is a reference of proxy contracts for wallet addresses. Here you can check if you have a proxy
...
**ETH Gas** - Setting the gas limit to a higher number, as we will interact with a contract.

**ETH Gas Price** - Setting the gas price to a higher number.

## Setting up or retrieve your proxy contract

  

There are many ways in interacting with the Oasis Direct Proxy contract. You could:

-   Directly call functions to the Oasis Direct Proxy
...
If you choose to use DS-Proxy, then you give allowance only once and this contract can interact with any other proxy actions contracts without having to give another allowance again.
...
Execute the below command to verify if you have a proxy contract:

`seth call $PROXY_REGISTRY 'proxies(address)(address)' $ETH_FROM`  

If output is an address (for instance `3704fa2f6234ad5f2da063121496003e999c5312`), then this address is your proxy contract. If you get an output like this: `0000000000000000000000000000000000000000` then you don’t have one.
...
`export MYPROXY=<YOUR_PROXY_ADDRESS>`

## Oasis Direct Proxy Functions

These functions are used for making an instant market order on the exchange. If you want to see a frontend example, check out the instant page on [oasis.app/trade/instant](https://oasis.app/trade/instant). If you want to make a simple limit order, then check the OasisDEX protocol [contract](https://github.com/makerdao/maker-otc).
...
If you want to understand each function in more detail, please do read the provided [README](https://github.com/makerdao/oasis-direct-proxy). For example, if you wanted to sell exactly 10 BAT for some amount of ETH, you would use sellAllAmountBuyEth(). If you wanted to sell at least 10 ZRX for some specified amount of DAI, you would use buyAllAmount() as you will be buying specific amount of DAI, paying for it with ZRX.
...
Approve Dai in MYPROXY, here you approve an unlimited amount to be taken from your wallet.
...
With DS-Proxy you only have to approve the tokens contracts once in order to carry as many exchanges as you want. This is the benefit of using such a contract.
...
Before executing the function to the proxy contract, you need to build the calldata with the function name and all its parameters. You can export all this calldata to an env variable in the terminal.

Set the buyAmt parameter to 3 Dai. `eth` in the `seth --to-wei 3 eth` command refers to the ETH conversion unit. It adds 18 zeros to 3. Converting it to the wei format.
...
If you look closely, you’ll notice that you sent more ETH than was necessary to the exchange. For getting 3 Dai you paid 0.037974683544303797 ETH and the rest was returned back to your wallet. This is special to this exchange function. You defined the exact number of Dai you want to receive. Depending on the market liquidity, you might spend more or less ETH for the Dai. So, in your case you bought Dai at the ~ 79 ETH/DAI price.

As you observed, all this has been done through one transaction on the Ethereum blockchain. This leads to improved UX, and less transaction.
...
In this guide you have learned how to use your proxy contract to execute Oasis Direct Proxy actions on the OasisDEX protocol. Using a proxy contract can be beneficial for both the user and the developer creating a dApp in terms of making one transaction on the network and greater user experience.
