...
A simple DSR example of how to interact with the Maker core can be found [here](https://github.com/makerdao/developerguides/blob/master/dai/dsr/dsr.sol). Make sure to not use this code in production, as it has not been audited. In this example, you can see how to properly call the `[pot.join(x)](https://github.com/makerdao/developerguides/blob/master/dai/dsr/dsr.sol#L80)`, `[pot.exit(x)](https://github.com/makerdao/developerguides/blob/master/dai/dsr/dsr.sol#L87)` and `[pot.exitAll()](https://github.com/makerdao/developerguides/blob/master/dai/dsr/dsr.sol#L93)` functions. Give close attention to the helper math functions, as they convert the Dai ERC-20 token 18 decimal into the internal accounting 27 decimal numbers. This could be used as inspiration for services that have a pool of Dai interacting with the Dai Savings Rate. Again, it is important to note that the former example is not production ready code, but only for inspiration.
